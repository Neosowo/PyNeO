...
def actualizar_cantidad_total(producto, cantidad):
    ...
    # Actualizar subms con la suma de los valores totales
...
def trans():
    ...
    if not any([cantidad.get() for cantidad in cantidades]):
        messagebox.showwarning("Warning", "Enter at least one quantity.")
        return
    ...
    with open("reg.txt", "a") as f:
        ...
        for producto, cantidad_total in cantidad_total_por_producto.items():
            f.write(f"{producto}: {cantidad_total}\n")
...
tk.Label(owo, text="$$").grid(column=3, row=24)
tk.Label(owo, text="$$").grid(column=3, row=25)
tk.Label(owo, text="$$").grid(column=3, row=26)
